...
func _physics_process(delta):
	...
	if heuristic == "human":
		...
		var target_throttle = Input.get_axis("ui_down", "ui_up")

		if car:
			# Use lerp for smooth, gradual input response
			car.steer_input = lerp(car.steer_input, target_steer, delta * 5.0)
			car.throttle_input = lerp(car.throttle_input, target_throttle, delta * 3.0)
